...
//	private SceneTransition sceneTransition;
    ...
    {
		float variable_prueba;
		System.Random variable = new System.Random();
		variable_prueba = variable.Next(0, 10);

        if (accessedMapSettings.getEncounterList(encounterLocation).Length > 0)
        {
            if (variable_prueba <= accessedMapSettings.getEncounterProbability())
            {
